...
bool time_count_key = 0;
unsigned long time_count_1 =0;
int heat_count = 0;
int move_times = 0;
bool preventbackstatus = 0; //add by martin 2014-4-30
...
static void lcd_status_screen() {
  ...
  lcd_implementation_status_screen();
    ...
    feedrate_multiplier = constrain(feedrate_multiplier, 10, 300);
...
//  enqueuecommand("G1 X0 Y0 Z");
  // plan_buffer_line(20, 20, 20, 0, 60,active_extruder);
 //  enqueuecommand("G28");
SERIAL_ECHOLN("1");
// quickStop();
 ...
 *
 */
static void lcd_machine_information_screen(){
	//lcd.setCursor(1,0);
	//lcd.print(MACHINE_UUID);
	//lcd.setCursor(1,1);
	//lcd.print("Conrad");
	lcd.setCursor(1,2);
	lcd.print(" Version     1.0 ");
	//lcd.setCursor(1,3);
	//lcd.print(FIRMWARE_URL);
	if( preventbackstatus==1 && LCD_CLICKED){
		//SERIAL_ECHOLN("100");
		preventbackstatus == 0;
...
static void lcd_machine_information_menu(){
	
	//lcd.setCursor(1,2);
//	lcd.print("Firmware 0.10 ");
	preventbackstatus = 1;
  currentMenu = lcd_machine_information_screen;
//  preventbackstatus = 1;
...
static void lcd_main_menu() {
  ...
  if (movesplanned() || IS_SD_PRINTING) {
	...
	if(sdcard_oprate == 1  ){
	    MENU_ITEM(gcode, "Emergency stop",PSTR("M112"));
	}
  }
  else {
 ...
 // MENU_ITEM(submenu, MSG_CONTROL, lcd_control_menu);
    ...
    if (card.cardOK) {
     ...
     MENU_ITEM(function, "Emergency stop", kill);
      }
      else {
      if(sdcard_oprate == 0  ){
       //  SERIAL_ECHOLN("M112");
		...
		MENU_ITEM(function, "About",lcd_machine_information_menu);  // SD-card changed by user
	//	MENU_ITEM(function, "Version 1.0",lcd_machine_information_menu);  // SD-card changed by user
        }
        #if SDCARDDETECT < 1
...
//  MENU_ITEM(function, "Firmware Version", lcd_machine_information_menu);  // SD-card changed by user
...
static void lcd_tune_menu() {
  ...
  MENU_ITEM_EDIT(int3, MSG_SPEED, &feedrate_multiplier, 10, 300);
  #if TEMP_SENSOR_0 != 0
  ...
  #endif
  MENU_MULTIPLIER_ITEM_EDIT(int3, MSG_FAN_SPEED, &fanSpeed_t, 0, 255);
  MENU_ITEM_EDIT(int3, MSG_FLOW, &extruder_multiply[active_extruder], 10, 300);
  //MENU_ITEM_EDIT(int3, MSG_FLOW MSG_F0, &extruder_multiply[0], 10, 999);
  #if TEMP_SENSOR_1 != 0
    MENU_ITEM_EDIT(int3, MSG_FLOW MSG_F1, &extruder_multiply[1], 10, 300);
  #endif
  #if TEMP_SENSOR_2 != 0
    MENU_ITEM_EDIT(int3, MSG_FLOW MSG_F2, &extruder_multiply[2], 10, 300);
  #endif
  #if TEMP_SENSOR_3 != 0
    MENU_ITEM_EDIT(int3, MSG_FLOW MSG_F3, &extruder_multiply[3], 10, 300);
  #endif
  ...
  #endif
 // MENU_ITEM(gcode, "Kill printer",PSTR("M112"));
  END_MENU();
}
 ...
 *
 */
	static void lcd_move_e_1() {
	
		START_MENU();
		
		MENU_ITEM(back, MSG_MOVE_AXIS, lcd_prepare_menu);
		
		MENU_ITEM(gcode, "extrude 10 mm", PSTR("M986"));
		
		MENU_ITEM(gcode, "extrude 100 mm", PSTR("M987"));
		/*
		MENU_ITEM(gcode, "extrude 10 mm", PSTR("G1 E10 F150"));
		MENU_ITEM(gcode, "extrude 100 mm", PSTR("G1 E100 F150"));
		MENU_ITEM(gcode, "extrude 1000 mm", PSTR("G1 E1000 F150"));
		MENU_ITEM(gcode, "extrude -10 mm", PSTR("G1 E-10 F150"));
		MENU_ITEM(gcode, "extrude -100 mm", PSTR("G1 E-100 F150"));
		MENU_ITEM(gcode, "extrude -1000 mm", PSTR("G1 E-1000 F150"));
		*/
		END_MENU();
		/*
	  if (encoderPosition != 0) {
...
/*	START_MENU();
	...
	MENU_ITEM(gcode, "extrude -10 mm", PSTR("M989"));
	MENU_ITEM(gcode, "extrude -100 mm", PSTR("M988"));
