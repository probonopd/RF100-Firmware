...
void loop() {
  ...
  WRITE(X_MS1_PIN, 1);//WRITE(X_MS1_PIN, 0);//WRITE(X_MS1_PIN, 1);//----------------
  SET_OUTPUT(X_MS2_PIN);//-----------------
  WRITE(X_MS2_PIN, 1);//--------------------

  SET_OUTPUT(Y_MS1_PIN);//---------------------
  WRITE(Y_MS1_PIN, 1);//WRITE(Y_MS1_PIN, 0);//WRITE(Y_MS1_PIN, 1);//----------------
  SET_OUTPUT(Y_MS2_PIN);//-----------------
  WRITE(Y_MS2_PIN, 1);//-------------------- 
   
   SET_OUTPUT(E_MS1_PIN);//---------------------
  WRITE(E_MS1_PIN, 1);//WRITE(E_MS1_PIN, 0);//WRITE(E_MS1_PIN, 1);//----------------
  SET_OUTPUT(E_MS2_PIN);//-----------------
  WRITE(E_MS2_PIN, 1);//-------------------- 
   
   
  SET_OUTPUT(Z_MS1_PIN);//---------------------
  WRITE(Z_MS1_PIN, 0);//WRITE(Z_MS1_PIN, 1);//WRITE(Z_MS1_PIN, 0);//----------------
  SET_OUTPUT(Z_MS2_PIN);//-----------------
  WRITE(Z_MS2_PIN, 1);//WRITE(Z_MS2_PIN, 0);//WRITE(Z_MS2_PIN, 1);//--------------------
...
void get_command() {
  ...
  while (MYSERIAL.available() > 0 && commands_in_queue < BUFSIZE) {
    ...
    if (serial_char == '\n' || serial_char == '\r' ||
      ...
      if (strchr(command, 'G') != NULL) {sdcard_oprate = 1;
